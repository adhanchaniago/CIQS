...
class CdController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','sampul'),
				'users'=>array('@'),
			),
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Cd'])) {
			...
			if ($model->save()) {
				...
				$this->redirect(array('view','id'=>$model->kode_cd,'asDialog'=>$_GET['asDialog']));
			}
		}
	...
	{
		/*$dataProvider=new CActiveDataProvider('Cd');
	 ...
	 * @param Cd $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if (isset($_POST['ajax']) && $_POST['ajax']==='cd-form') {
			echo CActiveForm::validate($model);
			Yii::app()->end();
		}
	}
	
	public function actionSampul($id)
	{
		...
		if (!empty($file)) {
			...
			header('Content-Disposition: attachment; filename='.'"'.$file->nama_sampul .'"');
