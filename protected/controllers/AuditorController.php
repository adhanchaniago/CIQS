...
class AuditorController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Auditor'])) {
			$model->attributes=$_POST['Auditor'];
	...
	{
		/*$dataProvider=new CActiveDataProvider('Auditor');
		$this->render('index',array(
			'dataProvider'=>$dataProvider,
		));*/
		$model=new Auditor('search');
		$model->unsetAttributes();  // clear any default values
		if (isset($_GET['Auditor'])) {
			$model->attributes=$_GET['Auditor'];
		}
	...
	{
		$model=Auditor::model()->findByPk($id);
		if ($model===null) {
	 ...
	 * @param Auditor $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if (isset($_POST['ajax']) && $_POST['ajax']==='auditor-form') {
